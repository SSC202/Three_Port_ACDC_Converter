...
{
    sogi->w0      = w0;
    sogi->tsample = t_sample;

    // 参数计算
    sogi->d0 = ((4) / (sogi->tsample * sogi->tsample)) + ((2 * sogi->w0) / (sogi->tsample)) + (sogi->w0 * sogi->w0);
    sogi->d1 = -((8) / (sogi->tsample * sogi->tsample)) + (2 * (sogi->w0 * sogi->w0));
    sogi->d2 = ((4) / (sogi->tsample * sogi->tsample)) - ((2 * sogi->w0) / (sogi->tsample)) + (sogi->w0 * sogi->w0);

    sogi->b0_alpha = ((2) * (sogi->w0)) / (sogi->tsample * sogi->d0);
    sogi->b0_beta  = ((sogi->w0) * (sogi->w0)) / (sogi->d0);
    sogi->b1_beta  = sogi->b0_beta * 2.0f;

    sogi->a1 = sogi->d1 / sogi->d0;
    sogi->a2 = sogi->d2 / sogi->d0;
...
void SOGI_Calc(SOGI_t *sogi)
{
    ...
    /************* alpha 通道计算 **************/
    // 更新步
    sogi->y_alpha[2] = sogi->y_alpha[1];
    sogi->y_alpha[1] = sogi->y_alpha[0];
    // 计算步
    sogi->y_alpha[0]   = sogi->b0_alpha * sogi->u[0] - sogi->b0_alpha * sogi->u[2] - sogi->a2 * sogi->y_alpha[2] - sogi->a1 * sogi->y_alpha[1];
    sogi->output_alpha = sogi->y_alpha[0];

    /************* beta 通道计算 **************/
    // 更新步
    sogi->y_beta[2] = sogi->y_beta[1];
    sogi->y_beta[1] = sogi->y_beta[0];
    // 计算步
    sogi->y_beta[0]   = sogi->b0_beta * sogi->u[0] + sogi->b1_beta * sogi->u[1] + sogi->b0_beta * sogi->u[2] - sogi->a2 * sogi->y_beta[2] - sogi->a1 * sogi->y_beta[1];
    sogi->output_beta = sogi->y_beta[0];
}
...
void QPR_Init(QPR_t *qpr, float wc, float wr, float t_sample, float kp, float kr)
{
    ...
    qpr->kp      = kp;
    qpr->kr      = kr;
    qpr->tsample = t_sample;

    qpr->d0 = (qpr->tsample * qpr->tsample * qpr->wr * qpr->wr) + (4 * qpr->wc * qpr->tsample) + 4.0f;

    qpr->a1 = (2 * qpr->tsample * qpr->tsample * qpr->wr * qpr->wr - 8.0f) / qpr->d0;
    qpr->a2 = (qpr->tsample * qpr->tsample * qpr->wr * qpr->wr - 4 * qpr->wc * qpr->tsample + 4.0f) / qpr->d0;

    qpr->b0 = (4 * qpr->kp + qpr->tsample * qpr->tsample * qpr->kp * qpr->wr * qpr->wr + 4 * qpr->tsample * qpr->kp * qpr->wc + 4 * qpr->tsample * qpr->kr * qpr->wc) / qpr->d0;
    qpr->b1 = (2 * qpr->kp * qpr->tsample * qpr->tsample * qpr->wr * qpr->wr - 8 * qpr->kp) / qpr->d0;
    qpr->b2 = (4 * qpr->kp + qpr->tsample * qpr->tsample * qpr->kp * qpr->wr * qpr->wr - 4 * qpr->tsample * qpr->kp * qpr->wc - 4 * qpr->tsample * qpr->kr * qpr->wc) / qpr->d0;
...
void QPR_Calc(QPR_t *qpr, uint8_t enable)
{
    ...
    qpr->u[2] = qpr->u[1];
    qpr->u[1] = qpr->u[0];
    qpr->u[0] = qpr->ref - qpr->fdb;

    qpr->y[2] = qpr->y[1];
    qpr->y[1] = qpr->y[0];
    qpr->y[0] = qpr->b0 * qpr->u[0] + qpr->b1 * qpr->u[1] + qpr->b2 * qpr->u[2] - qpr->a1 * qpr->y[1] - qpr->a2 * qpr->y[2];
